...
namespace SmashBalloon\TikTokFeeds\Common\UsageTracking\Core;
...
class Sender
{
	 ...
	 *             made (empty URL, encode failure, oversize payload, transport
	 *             error). Callers treat 2xx as success; auth failures mean the
	 *             site token should be re-registered.
	 */
	public function send(array $payload)
	{
		...
		if ('' === Config::get_api_url()) {
			// Constant undefined or blanked via the sbtt_smash_usage_tracking_api_url
			// filter — a relative URL would make wp_remote_post() fail anyway.
			return 0;
		}
		...
		$max_bytes = (int) apply_filters('sbtt_smash_usage_tracking_max_payload_bytes', Config::MAX_PAYLOAD_BYTES);
		if ($max_bytes > 0 && strlen($body) > $max_bytes) {
			if (defined('WP_DEBUG') && WP_DEBUG && defined('WP_DEBUG_LOG') && WP_DEBUG_LOG && function_exists('error_log')) {
				error_log('[SBTT Usage Tracking] Payload size ' . strlen($body) . ' exceeds max ' . $max_bytes . ', send skipped.');
			}
			return 0;
		}

		$timeout = (int) apply_filters('sbtt_smash_usage_tracking_request_timeout', Config::REQUEST_TIMEOUT);
		...
		$response = wp_remote_post(
			...
			array(
				...
				'body'        => $body,
				'user-agent'  => 'SBTT/' . (defined('SBTTVER') ? SBTTVER : '') . '; ' . get_bloginfo('url'),
			)
		);
	 ...
	 * @return bool
	 */
	public function last_error_rejected_token($code)
	{
		if (in_array((int) $code, array( 401, 403, 410 ), true)) {
			return true;
		}
		if (422 !== (int) $code || '' === $this->last_error_body) {
			return false;
		}
